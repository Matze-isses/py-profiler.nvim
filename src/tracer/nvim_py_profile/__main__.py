...
class Tracer:
    ...
    def __init__(self, script_of_interest):
        ...
        self._processor = Processor(self._logger, self._home_dir)
        self._send = Sender()
    ...
    def __call__(self, frame, event, args):
        ...
        if frame.f_code.co_filename not in self._seen_files:
            self._seen_files.append(frame.f_code.co_filename)
        ...
        for disqualifier in ["def", "return", "for", "while", "import", "class", "@", "if"]:
            if disqualifier in line: return self.__call__
...
if __name__ == "__main__":
    ...
    runpy.run_path(script_of_interest)
